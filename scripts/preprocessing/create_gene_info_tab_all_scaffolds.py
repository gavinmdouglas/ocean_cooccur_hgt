...
# Include genes on all scaffolds.
...
def main():
    ...
    # Read through all Prokka GFF files and identify  which genes are on those scaffolds.
    gff_filepaths = list()
        ...
        with open(gff_filepath, 'r') as gff_fh:
            for line in gff_fh:
                if line.startswith('#'):
                    ...
                    print(gene_id + '\t' + rep_gene + '\t' + line_split[0] + '\t' + genome_id)

    print('Number of genomes ignored: ' + str(failed_genome), file = sys.stderr)
    print('Number of genomes retained: ' + str(passed_genome), file = sys.stderr)
    print('Number of genes where no gene representative was found: ' + str(no_gene_rep) + ' out of ' + str(total_genes) + ' total genes', file = sys.stderr)
