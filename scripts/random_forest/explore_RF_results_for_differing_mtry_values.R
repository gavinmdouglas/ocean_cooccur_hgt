...
# Make sure that RF variable importance results don't change much if different mtry values are used.
...
for (mtry_val in 1:11) {
  set.seed(97026)
  rf_ranger <- ranger(prop_hgt ~ ., data = rf_input, importance = "permutation", num.trees = num_trees, mtry = mtry_val)
  ...
  plot(ggplot(varimp_plot_df, aes(x = variable, y = importance, fill = association)) +
    geom_col() +
