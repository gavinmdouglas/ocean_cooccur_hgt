...
# Get RF output with sampling method included, to confirm that this does not change the overall conclusion
# (and that sampling method itself is not a significant predictor after controlling for other variables)
...
for (varname in colnames(cleaned_meta)) {
  if (varname != "Sampling_method") {
    ...
    spearman_cor <- c(spearman_cor, NA)
    spearman_p <- c(spearman_p, NA)
  }
}
...
rf_input$Sampling_method <- factor(rf_input$Sampling_method)
