...
# Then, read through metadata file again, and figure out which samples are
# the sole samples for a given run ID, and which have multiple.
# Write out symbolic link and cat commands, as needed.
parsed_runs = set()
    ...
    for line in metadata_fh:
        ...
        if len(sample_runids) == 1:
            ...
            if len(run_fastqs) == 1:
                ...
                if exp_se != run_fastqs[0]:
                    sys.exit("Error: unexpected filename for: " + runid)
                outfile_se = sample_id + ".fastq.gz"
                ln_cmds.append("ln -s /mfs/gdouglas/projects/water_mags/additional/filtered/" + exp_se + " /mfs/gdouglas/projects/water_mags/additional/filtered_clean/" + outfile_se)
                se_samples.append(sample_id)
            elif len(run_fastqs) == 2:
                exp_r1 = runid + "_1.fastq.gz"
                exp_r2 = runid + "_2.fastq.gz"
                if exp_r1 not in run_fastqs or exp_r2 not in run_fastqs:
                    sys.exit("Error: unexpected filenames for: " + runid)
                outfile_r1 = sample_id + "_1.fastq.gz"
                outfile_r2 = sample_id + "_2.fastq.gz"
                ln_cmds.append("ln -s /mfs/gdouglas/projects/water_mags/additional/filtered/" + exp_r1 + " /mfs/gdouglas/projects/water_mags/additional/filtered_clean/" + outfile_r1)
                ln_cmds.append("ln -s /mfs/gdouglas/projects/water_mags/additional/filtered/" + exp_r2 + " /mfs/gdouglas/projects/water_mags/additional/filtered_clean/" + outfile_r2)
            ...
            for runid in sample_runids:
                ...
                if len(run_fastqs) == 1:
                    exp_se = runid + ".fastq.gz"
                    if exp_se != list(run_fastqs)[0]:
                        sys.exit("Error: unexpected filename for: " + runid)
                    se_fastqs.append("/mfs/gdouglas/projects/water_mags/additional/filtered/" + exp_se)
                elif len(run_fastqs) == 2:
                    exp_r1 = runid + "_1.fastq.gz"
                    exp_r2 = runid + "_2.fastq.gz"
                    if exp_r1 not in run_fastqs or exp_r2 not in run_fastqs:
                        sys.exit("Error: unexpected filenames for: " + runid)
                    r1_fastqs.append("/mfs/gdouglas/projects/water_mags/additional/filtered/" + exp_r1)
                    r2_fastqs.append("/mfs/gdouglas/projects/water_mags/additional/filtered/" + exp_r2)
                else:
            ...
            if len(se_fastqs) > 0 and (len(r1_fastqs) > 0 or len(r2_fastqs) > 0):
                ...
                cat_cmds.append("cat " + " ".join(se_fastqs) + " > /mfs/gdouglas/projects/water_mags/additional/filtered_clean/" + sample_id + ".fastq.gz")
                se_samples.append(sample_id)
                ...
                cat_cmds.append("cat " + " ".join(r1_fastqs) + " > /mfs/gdouglas/projects/water_mags/additional/filtered_clean/" + sample_id + "_1.fastq.gz")
                cat_cmds.append("cat " + " ".join(r2_fastqs) + " > /mfs/gdouglas/projects/water_mags/additional/filtered_clean/" + sample_id + "_2.fastq.gz")
                pe_samples.append(sample_id)
            else:
                sys.exit("No FASTQs for sample??: " + sample_id)

# Write out symbolic link and cat commands.
#with open("/mfs/gdouglas/projects/water_mags/additional/filtered_clean_ln_cmds.sh", "w") as ln_fh:
#    for ln_cmd in ln_cmds:
#        ln_fh.write(ln_cmd + "\n")

#with open("/mfs/gdouglas/projects/water_mags/additional/filtered_clean_cat_cmds.sh", "w") as cat_fh:
