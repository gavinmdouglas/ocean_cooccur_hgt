...
BLASTn_subset_col_i <- grep("BLASTn", colnames(combined_COG))

BLAST_COG_glmm_out <- run_glmms_per_grouping(in_tab = combined_COG,
                               subset_col_i = BLASTn_subset_col_i,
...
BLAST_proMGE_glmm_out <- run_glmms_per_grouping(in_tab = combined,
                                             subset_col_i = BLASTn_subset_col_i,
...
BLAST_crispr_hit_glmm_out <- run_glmms_per_grouping(in_tab = combined,
                                                subset_col_i = BLASTn_subset_col_i,
...
BLAST_genomad_glmm_out <- run_glmms_per_grouping(in_tab = combined,
                                                    subset_col_i = BLASTn_subset_col_i,
...
BLAST_scaffold_contains_provirus_glmm_out <- run_glmms_per_grouping(in_tab = combined,
                                                     subset_col_i = BLASTn_subset_col_i,
...
model_summaries <- list(COG_category = lapply(BLAST_COG_glmm_out, summary),
                        proMGE = lapply(BLAST_proMGE_glmm_out, summary),
                        crispr = lapply(BLAST_crispr_hit_glmm_out, summary),
                        genomad = lapply(BLAST_genomad_glmm_out, summary),
                        scaffold_w_prophage = lapply(BLAST_scaffold_contains_provirus_glmm_out, summary))

saveRDS(object = model_summaries,
        file = '/mfs/gdouglas/projects/ocean_mags/glmm_working/blast_glmm_model_summaries.rds')
