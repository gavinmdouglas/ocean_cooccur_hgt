...
cooccur_parallel <- function (mat, type = "spp_site", thresh = TRUE, spp_names = FALSE, 
                              ...
                              cores = 1) 
{
  ...
  if (!is.null(site_mask)) {
    ...
    } else { stop("Incorrect dimensions for site_mask, aborting.")  }
  } else {
    site_mask <- matrix(data = 1,
                        nrow = nrow(spp_site_mat), 
                        ncol = ncol(spp_site_mat))

    N_matrix <- matrix(data = ncol(spp_site_mat),
                       nrow = nrow(spp_site_mat), 
  ...
  for (spp in 1:(nspp - 1)) {
      for (spp_next in (spp + 1):nspp) {
        print(c(site_mask[spp, ], site_mask[spp_next, ], mat[spp, ]))
