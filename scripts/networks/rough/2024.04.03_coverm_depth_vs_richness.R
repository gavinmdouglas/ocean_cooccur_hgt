...
for (bioproject in bioprojects) {
  ...
  for (file in all_files) {
    ...
    richness <- length(which(cov_in$breadth >= 0.25))
    
    orig_col <- colnames(cov_in)
    
    length_kb <- cov_in$length / 1000
    num_reads <- cov_in$rpkm * length_kb * 1e6
    
    mgs_id <- gsub('\\..*$', '', basename(file))
    cov_in$mgs_sample <- mgs_id

    raw_in[[paste(bioproject, mgs_id)]] <- data.frame(bioproject=bioproject, mgs_sample=mgs_id, richness=richness, depth=)
  }
