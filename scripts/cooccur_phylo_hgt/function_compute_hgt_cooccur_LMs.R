...
compute_hgt_cooccur_LMs_w_protist_grouping <- function(prepped_table_path,
                                                       outfolder,
                                                       num_cores=8,
                                                       protist_genomes) {
  ...
  outfolder="~/tmp/"
  intab <- read.table(prepped_table_path, header=TRUE, sep="\t", stringsAsFactors = FALSE)
  protist_genomes='~/tmp/protist_assoc_taxa.txt'

  protist_ids <- read.table(file = lessfiltered_genomes_file, header=FALSE, stringsAsFactors = FALSE)$V1
