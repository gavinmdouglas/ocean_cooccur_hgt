...
# Plot distributions of tip distance and co-occurrence based on pairwise comparisons of
hyperg_combined_info <- read.table("/mfs/gdouglas/projects/ocean_mags/networks/allsamples/blast/metaG_hyperg_cooccur.allsamples.combined.tsv.gz",
...
hyperg_combined_info$hgt[which(hyperg_combined_info$both_hit_count > 0)] <- 'Yes'
...
combined_row <- plot_grid(contingency_heatmap, tip_dist_by_cooccur_and_hgt,
                          labels = c('a', 'b'), nrow = 1, rel_widths = c(1, 2))

ggsave(plot = combined_row,
       filename = "/mfs/gdouglas/scripts/ocean_mag_hgt/display_items/Main_BLASTn_hgt_cooccur_tipdist_overview.pdf",
       device = "pdf", width = 12.5, height = 4, units = "in", dpi=600)



# Same, but for cluster-based results.
