...
combined <- data.frame(matrix(NA, nrow = 5, ncol = 6))
colnames(combined) <- rev(blast_hits$Inter.level)
rownames(combined) <- c("clusterbased_95", "clusterbased_99", "blast_95", "blast_99", "num_comparisons")

combined["clusterbased_95", ] <- clusterbased_hits$cluster_95_99
combined["clusterbased_99", ] <- clusterbased_hits$cluster_99
combined["blast_95", ] <- rev(blast_hits$blast95_99)
combined["blast_99", ] <- rev(blast_hits$blast99)
...
row_split_categories <- factor(c("Cluster-based HGT", "Cluster-based HGT", "BLASTn-based HGT", "BLASTn-based HGT", ""),
                               levels=c("Cluster-based HGT", "BLASTn-based HGT", ""))
...
count_heatmap <- Heatmap(matrix = as.matrix(combined_percent),
                                   ...
                                   row_gap = unit(5, "mm"),
                                   row_title_rot=0,
                                   row_labels = c(">= 95% and < 99%", ">= 99%", ">= 95% and < 99%", ">= 99%", "Number of    \ngenome        \ncomparisons"),
...
# Panel b
blast_norm_hits <- blast_hits[, c("Inter.level", grep(".norm", colnames(blast_hits), value = TRUE))]
...
blast_norm_hits_long$Approach <- 'BLASTn-based HGT'
...
cluster_norm_hits_long$Approach <- 'Cluster-based HGT'
...
norm_hits_combined$Approach <- factor(norm_hits_combined$Approach, levels = c("Cluster-based HGT", "BLASTn-based HGT"))
...
# Combine plot.
combined_plot <- plot_grid(count_heatmap, NULL, norm_hits_plot, labels=c('a', '', 'b'), nrow=3, rel_heights = c(0.475, 0.01, 0.475))

ggsave(plot = combined_plot,
       filename = "/mfs/gdouglas/scripts/ocean_mag_hgt/display_items/Main_identity_tallies_and_prop.pdf",
       device = "pdf", width = 9, height = 10, units = "in", dpi=600)
