...
Tara_presence <- read.table(file = "/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/networks/combined_tables/metaG_presence_tara.tsv.gz",
...
mean_tab <- data.frame(matrix(NA,
                              nrow = ncol(Tara_presence),
                              ncol = length(vars_of_interest)))
colnames(mean_tab) <- vars_of_interest
rownames(mean_tab) <- colnames(Tara_presence)

for (taxon in colnames(Tara_presence)) {
  samples_w_taxon <- rownames(Tara_presence)[which(Tara_presence[, taxon] > 0)]
...
mean_gzfile_connection <-  gzfile("/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/networks/combined_tables/Tara_present_metadata_mean_by_sample.tsv.gz", "w")
...
rownames(median_tab) <- colnames(Tara_presence)
...
median_gzfile_connection <-  gzfile("/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/networks/combined_tables/Tara_present_metadata_median_by_sample.tsv.gz", "w")
