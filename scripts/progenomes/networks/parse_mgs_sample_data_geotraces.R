...
geotraces_presence <- read.table(file = "/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/networks/combined_tables/metaG_presence_geotraces.tsv.gz",
...
mean_tab <- data.frame(matrix(NA,
                              nrow = ncol(geotraces_presence),
                              ncol = length(vars_of_interest)))
colnames(mean_tab) <- vars_of_interest
rownames(mean_tab) <- colnames(geotraces_presence)

for (taxon in colnames(geotraces_presence)) {
  samples_w_taxon <- rownames(geotraces_presence)[which(geotraces_presence[, taxon] > 0)]
...
mean_gzfile_connection <-  gzfile("/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/networks/combined_tables/geotraces_present_metadata_mean_by_sample.tsv.gz", "w")
...
rownames(median_tab) <- colnames(geotraces_presence)
...
median_gzfile_connection <-  gzfile("/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/networks/combined_tables/geotraces_present_metadata_median_by_sample.tsv.gz", "w")
