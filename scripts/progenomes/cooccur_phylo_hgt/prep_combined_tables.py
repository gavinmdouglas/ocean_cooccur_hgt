...
''',
    ...
    parser.add_argument("--tip_distances", metavar="TIP_DIST", type=str,
                        ...
                        required=False,
                        default="/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/genome_prep/high_qual_genome_tip_dist.tsv.gz")

    parser.add_argument("--hgt_tab", metavar="HGT_TAB", type=str,
                        help="Path to HGT pairwise tallies (gzipped).",
                        required=False,
                        default="/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/clusters/gene_hgt_tallies.tsv.gz")
    ...
    taxa_tab = pd.read_table(filepath_or_buffer=args.taxa_tab, compression='gzip', header=0, index_col=0, sep='\t')
    ...
    # Make sure that at least some taxa_combinations are in hgt_tab index.
    found_hgt = False
    found_cooccur = False
    for taxa_pair in taxa_combinations:
        ...
        if taxa_combo in hgt_tab.index:
            found_hgt = True
        elif taxa_combo in cooccur_tab.index:
            found_cooccur = True
    if not found_hgt:
        sys.exit("No taxa combinations found in HGT table.")
    if not found_cooccur:
        sys.exit("No taxa combinations found in co-occurrence table.")
    ...
    for taxa_pair in taxa_combinations:
        ...
        taxon_i_taxid = int(taxon_i.split('_')[0].replace('g', ''))
        taxon_j_taxid = int(taxon_j.split('_')[0].replace('g', ''))
        taxon_i_levels = taxa_tab.loc[taxon_i_taxid, tax_levels]
        taxon_j_levels = taxa_tab.loc[taxon_j_taxid, tax_levels]
        for i in range(len(taxon_i_levels)):
            if taxon_i_levels.iloc[i] != taxon_j_levels.iloc[i]:
                diff_level = tax_levels[i]
                break

        if diff_level in ['NO_DIFF_FOUND', 'Strain', 'Species']:
            continue
