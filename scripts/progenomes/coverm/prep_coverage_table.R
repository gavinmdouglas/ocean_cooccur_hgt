...
coverm_all_filt <- coverm_all[which(coverm_all$genome %in% genomes_to_keep), ]

# Create presence tables.
dna_presence <- prep_presence_tab(cov_tab = coverm_all_filt,
                                  presence_cutoff = 0.30)

# Create RPKM tables, based on the same samples/genomes as in the presence tables.
dna_rpkm <- reshape2::dcast(data = coverm_all_filt,
...
write_gzip_table_w_rowcol(x = dna_presence,
                          outfile = '/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/networks/combined_tables/metaG_presence_allsamples.tsv.gz')

write_gzip_table_w_rowcol(x = dna_rpkm,
                          outfile = '/mfs/gdouglas/projects/ocean_mags/progenomes_analyses/networks/combined_tables/metaG_rpkm_allsamples.tsv.gz')
